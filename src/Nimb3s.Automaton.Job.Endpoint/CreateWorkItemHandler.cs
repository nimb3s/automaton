...
{
    public class CreateWorkItemHandler //: IHandleMessages<UserSubmittedWorkItemMessage>
    {
        ...
        public async Task Handle(UserSubmittedWorkItemMessage message, IMessageHandlerContext context)
        {
            ...
            log.Info($"MESSAGE: {nameof(UserSubmittedWorkItemMessage)}; HANDLED BY: {nameof(QueueJobHandler)}: {JsonConvert.SerializeObject(message)}");
        }
        #endregion
    }
}
